#!/usr/bin/env python3
"""Zip + publish Kimi gateway to the UEFN Ducky Store via uds_release.
...
Usage:
  ...
  py scripts/release.py --publish --changelog "v1: Kimi LLM gateway"
"""
...
def publish(zip_path: Path, *, category: str, changelog: str) -> None:
    ...
    release_args: dict = {
        "category": category,
        "changelog": changelog or "v1.0.0: Kimi LLM gateway (Moonshot OpenAI-compatible API)",
        "publish": True,
        "categories": ["plugins", "gateways"],
        "tags": ["kimi", "moonshot", "llm", "gateway", "api-key"],
        "name": "Kimi",
        "description": (
            "Kimi API (Moonshot Open Platform). After Install + Enable, Kimi appears under "
            "Settings → LLMs → Providers & Keys. Paste your API key from platform.kimi.ai."
        ),
    }
    if file_id:
